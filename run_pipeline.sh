...
if [ -f "src/promote_model.py" ]; then
    ...
    if [ $PROMOTE_STATUS -eq 0 ]; then
        ...
        echo -e "${YELLOW}⚠ Авто-продвижение не сработало, проверяем вручную...${NC}"
        
        # Показываем список версий
        python src/promote_model.py --model-name telco_churn_model --list
        
        # Пробуем продвинуть версию 1
        python src/promote_model.py --model-name telco_churn_model --version 1
        
        if [ $? -eq 0 ]; then
            echo -e "${GREEN}✓ Модель промотирована вручную${NC}"
        else
            echo -e "${YELLOW}⚠ Модель не соответствует критериям для продвижения${NC}"
        fi
    fi
else
