=======
# 🚂 Train Delay Prediction — End-to-End MLOps Pipeline

**Проект по MLOps команды 6**

**Git, CleanCode, feature engineering** BookWormDevid
**ML, Training, AI sloptometry** Meksiner
**Datasets, Frontend, Presentation** Le666Ly

End-to-end MLOps pipeline для предсказания задержки поездов с полным CI/CD, мониторингом и детекцией дрифта.

---

## Быстрый старт

### 1. Клонирование и .venv
```bash
git clone https://github.com/BookWormDevid/T25--4m_MLOp.git
cd T25--4m_MLOp
uv venv .venv
source .venv/bin/activate  # Linux/Mac
# .venv\Scripts\activate   # Windows
```

### 2. Установка зависимостей
```bash
uv sync
uv sync --all-groups # Если хотите менять код
python tests/test_tf.py # проверка установки tensorflow
# если есть проблемы с tensorflow на виндовс, установите VC++ Redist
# uv pip uninstall tensorflow keras -y
# uv pip install tensorflow==2.18.1
```

### 3. DVC и датасет
```bash
curl -L -o data/raw/delay.csv https://www.kaggle.com/api/v1/datasets/download/bartek358/train-delays
unzip data/raw/delay.zip -d data/raw
rm -r data/raw/delay.zip
mlflow server --backend-store-uri sqlite:///mlflow/mlflow.db --default-artifact-root ./mlruns --host 0.0.0.0 --port 5000
dvc repro
```

### 4. Команды запуска и разное
#### 4.1 Данные и обучение
```bash
python tests/test_data.py
python src/train.py # обучение
```
#### 4.2 Запуск API
```bash
python -m uvicorn src.api:app --reload
```


## 📊 Проблематика

Цель проекта — **предсказать, время на которое будет задержан поезд**, используя исторические и контекстные данные.

Задача формулируется как **Sequential**:
float(delay) 
---

## 🔍 Используемые признаки

Модель обучается на следующих группах признаков:

### 🕒 Временные признаки
- Время прибытия
- Дата

### 🚉 Маршрут и инфраструктура
- Станция прибытия
- Маршрут
- Название ветки
- Тип маршрута

### 🚆 Поезд
- Тип поезда (региональный / скоростной / грузовой и т.д.)

### 🌦️ (Опционально) Погода (ПОКА НЕ ИСПОЛЬЗУЕТСЯ!!!!!)
- Температура
- Осадки
- Ветер

---

## 🛠️ Технологический стек

### 🔧 Основные технологии
- **Python 3.12**
- **uv** — управление зависимостями
- **Tensorflow** - обучение модели

### 📦 Работа с данными и ML
- **DVC** — версионирование данных
- **MLflow** — трекинг экспериментов и моделей
- **Feast** — feature store (WIP)

### 🧠 Оркестрация и пайплайны
- **Airflow** — оркестрация ETL и ML-пайплайнов (WIP)

### 🌐 API и деплой
- **FastAPI** — REST API для инференса
- **Docker** — контейнеризация
- **Kubernetes (Minikube)** — деплой и масштабирование (WIP)

### 📈 Мониторинг и качество (WIP)
- **Prometheus** — сбор метрик 
- **Grafana** — визуализация
- **Evidently** — детекция data drift / concept drift

### 🚀 CI/CD
- **GitHub Actions** — автоматизация:
  - тестов
  - сборки Docker-образов  (WIP)
  - деплоя  (WIP)
  - обновления моделей  (WIP)

---

## 🔄 MLOps Pipeline (E2E)

1. **Сбор данных**
   - Исторические данные поездов
   - (Временно пропущенно) погодные данные

2. **Версионирование данных**
   - DVC

3. **Feature Engineering**
   - Обработка временных и категориальных признаков
   - Загрузка признаков в Feast (WIP) (сейчас загружаются напрямую в processed)

4. **Обучение модели**
   - Логирование экспериментов в MLflow
   - Сравнение метрик
   - Регистрация лучшей модели  (WIP)

5. **CI/CD**
   - Автоматические тесты
   - Сборка Docker-образов  (WIP)
   - Деплой в Kubernetes  (WIP)

6. **Инференс**
   - FastAPI сервис
   - Online predictions  (WIP)

7. **Мониторинг**
   - Метрики сервиса и модели   (WIP)
   - Prometheus + Grafana   (WIP)

8. **Drift Detection** 
   - Evidently  (WIP)
   - Алерты при деградации данных или модели (WIP)

---

## 📁 Структура проекта

```text
.
├── feast/                 # Feature Store
├── api/                   # FastAPI сервис
├── airflow                # DAG'и Airflow
├── data/                  # DVC-tracked данные
├── dvc_remote/
├── feature_repo/          # Feature engineering
├── .github/workflows/     # CI/CD (GitHub Actions)
├── k8s/                   # Kubernetes манифесты
├── mlflow/
├── models/
├── docker/                # docker-compose && dockerfile
├── monitoring/            # Prometheus / Evidently
├── reports/               # EDA и подобное
├── src/                   # Основные скрипты проекта
├── tests/                 # Unit-testы и проверки установки
├── dvc.yaml
├── environment.yml        # Совместимость и удобство для conda
├── pyproject.toml         # Зависимости
├── requirements.txt       # Зависимости без UV

