...
def test_preprocess_creates_output(tmp_path):
    ...
    subprocess.run(
        ["python", "src/preprocess.py", str(raw_dir), str(processed_dir)], check=True
    )
    ...
    assert not df_out.empty, "Выходной CSV пуст"
    assert "target" in df_out.columns, "Нет столбца target"
